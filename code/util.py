...
def normalize(voxelArrayMap):
  numFeatures = len(voxelArrayMap[voxelArrayMap.keys()[0]])
  m = len(voxelArrayMap)
  ...
  for v in range(Constants.NUM_VOXELS):
    for cat in range(15):
      for run in range(Constants.NUM_RUNS):
        imageCat = cat / 3
        if cat % 3 == 0:
          oddBall[v][imageCat][run] = sub_Betas[v][cat][run]
        elif cat %3 == 1:
          workingMemory[v][imageCat][run] = sub_Betas[v][cat][run]
        else:
          selectiveAttention[v][imageCat][run] = sub_Betas[v][cat][run]
  return [(oddBall, 'oddBall', subjectKey), (workingMemory, 'workingMemory', subjectKey), (selectiveAttention, 'selectiveAttention', subjectKey)]
...
def getVoxelArray(allTasks=True, includeOddBall=False, includeWorkingMemory=False, includeSelectiveAttention=True,
  ...
  if containsOne(subjectNumbers):
    mat = scipy.io.loadmat('../data/sub1_betas.mat')
...
#amplitude = M[vNum][imageTupe][run]
  ...
  #   if includeSelectiveAttention: arr.append(selectiveAttentionOnlyTasks)
  #taskMatrix = vNumber, cat, run
  voxelArrayMap = {}
