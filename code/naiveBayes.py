...
#key = (run, image type, task name, subject number)
    ...
    for category in range(self.numCategories):
      for testIndex, pair in enumerate(self.testExamples):
        ...
        for i, voxel in enumerate(value):
          ...
          probability += math.log(pdf) # voxel * math.log(self.averages[category][i]) + (1 - voxel) * math.log(1 - self.averages[category][i])
...
if __name__ == '__main__':
  ...
  # classification = [[0 for _ in range(Constants.NUM_CATEGORIES)] for _ in range(Constants.NUM_CATEGORIES)]
  # for i in range(Constants.NUM_CATEGORIES):
  #   for j in range(Constants.NUM_CATEGORIES):
  #     if i == j: classification[i][j] = 100
  #     if i > j: classification[i][j] = classification[j][i]
  #     else: classification[i][j] = run([i, j], [4])
  # g = grapher.Grapher()
  # g.twoDHeatMap(classification, vmin=0, vmax=100, title='Naive Bayes', xLabel='Category Index', yLabel='Category Index')
